...
type WorkerMessage =
  ...
  | SetIndexingWorkerPortMessage
  | SetDbWorkerPortMessage;
...
function readFileForIndexingWorker(message: any) {
  ...
  unzip(
    ...
    (err, files) => {
      if (err) {
        console.error(
          "🔧 Zip worker: Unzip failed for indexing worker:",
          err.message,
        );
        if (indexingWorkerPort) {
          indexingWorkerPort.postMessage({
      ...
      try {
        ...
        console.error(
          "🔧 Zip worker: Error processing file for indexing worker:",
          fileError,
        );
        if (indexingWorkerPort) {
          indexingWorkerPort.postMessage({
...
function readFileForDbWorker(message: any) {
  ...
  unzip(
    ...
    (err, files) => {
      if (err) {
        console.error(
          "🔧 Zip worker: Unzip failed for DB worker:",
          err.message,
        );
        if (dbWorkerPort) {
          dbWorkerPort.postMessage({
      ...
      try {
        ...
        console.error(
          "🔧 Zip worker: Error processing file for DB worker:",
          fileError,
        );
        if (dbWorkerPort) {
          dbWorkerPort.postMessage({
