...
const TEXT_EXTENSIONS = new Set([
  ...
  'py', 'rb', 'go', 'java', 'c', 'cpp', 'h', 'hpp', 'cs', 'php', 'sh', 'bash',
  'sql', 'csv', 'tsv', 'log', 'yaml', 'yml', 'toml', 'ini', 'cfg', 'conf',
  'proto', 'graphql', 'dockerfile', 'makefile', 'gitignore', 'license',
  'readme', 'changelog', 'manifest', 'properties', 'env'
]);

export function detectBinaryByExtension(filename: string): BinaryDetectionResult {
...
export function detectBinaryByContent(content: string, maxBytes: number = 4096): BinaryDetectionResult {
  if (!content) {
...
export function detectBinary(filename: string, content?: string): BinaryDetectionResult {
...
export function getFileTypeDescription(result: BinaryDetectionResult, filename: string): string {
  if (result.fileType) {
