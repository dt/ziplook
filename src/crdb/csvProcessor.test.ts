    ...
    const testCases = [
      {
        ...
        input: {
          ...
          value:
            "\\x0a126b65792076697375616c697a6572206a6f6212046e6f646518f98edbe2e3e08f03a001019a0210209f0491db3e4a1d96b30aa3640ca611a20208081910031800200cb20200",
        },
        expectedProto: {
      ...
      },
      {
        ...
        input: {
          ...
          value:
            "\\x0a0f504f4c4c204a4f425320535441545312046e6f646518b2e9dae2e3e08f03a001019a0210209f0491db3e4a1d96b30aa3640ca611a20208081910031800200cba0200",
        },
        expectedProto: {
          creationClusterId: "209f0491-db3e-4a1d-96b3-0aa3640ca611",
          creationClusterVersion: {
      ...
      },
      {
        ...
        input: {
          ...
          value:
            "\\x0a1073716c206163746976697479206a6f6212046e6f646518cd97dbe2e3e08f03a001019a0210209f0491db3e4a1d96b30aa3640ca611a20208081910031800200ce20200",
        },
        expectedProto: {
      ...
      },
      {
        ...
        input: {
          ...
          value:
            "\\x0a1a6d766363207374617469737469637320757064617465206a6f6212046e6f646518c8ebdae2e3e08f03a001019a0210209f0491db3e4a1d96b30aa3640ca611a20208081910031800200cea0200",
        },
        expectedProto: {
          creationClusterId: "209f0491-db3e-4a1d-96b3-0aa3640ca611",
          creationClusterVersion: {
      ...
      },
      {
        ...
        input: {
          ...
          value:
            "\\x0a",
        },
        expectedProto: {
          creationClusterId: "209f0491-db3e-4a1d-96b3-0aa3640ca611",
          creationClusterVersion: {
          ...
          statement: [
            "CREATE STATISTICS __auto__ FROM [3] WITH OPTIONS THROTTLING 0.9 AS OF SYSTEM TIME '-30s'",
          ],
          usernameProto: "node",
        },
      },
      {
        ...
        input: {
          ...
          value:
            "\\x0a1f7265636f6e63696c696e67207370616e20636f6e66696775726174696f6e7312046e6f6465188afb8ed5e3e08f03a001019a0210209f0491db3e4a1d96b30aa3640ca611a20208081910031800200cda0100",
        },
        expectedProto: {
  ...
  describe("Proto message type detection", () => {
    it("should use correct proto type based on info_key", () => {
      const payloadCases = [
        {
          info_key: "legacy_payload",
          expectedType: "cockroach.sql.jobs.jobspb.Payload",
        },
        {
          info_key: "legacy_progress",
          expectedType: "cockroach.sql.jobs.jobspb.Progress",
        },
      ];
  ...
  testCases.forEach(({ fileName, expectedJsonColumns, description }) => {
    it(description, async () => {
      ...
      expect(lines.length).toBeGreaterThan(
        1,
        `File ${fileName} should have header + data rows`,
      );
      ...
      if (lines.length > 34) {
        ...
        console.log(
          `  Debug: Row 34 has descriptor ID=${row34[0]}, hex length=${row34[1]?.length || 0}`,
        );
      }
      ...
      console.log(
        `Processing ${fileName}: ${processedLines.length - 1} data rows`,
      );
      ...
      for (let i = 1; i < processedLines.length; i++) {
          ...
          if (value.startsWith("{") && value.endsWith("}")) {
            try {
              ...
              if (typeof parsed === "object" && parsed !== null) {
                jsonColumnCount++;
                console.log(
                  `Row ${i}: Found JSON in column ${headers[j]}: ${value.substring(0, 100)}...`,
                );
              }
            } catch (e) {
        ...
        if (hasProtobufData && jsonColumnCount === 0) {
          console.log(
            `Row ${i}: Has protobuf data but NO JSON conversion - conversion failed`,
          );
        } else if (!hasProtobufData && jsonColumnCount === 0) {
          console.log(`Row ${i}: No protobuf data, no JSON - normal`);
        } else if (jsonColumnCount > 0) {
          console.log(
            `Row ${i}: Successfully converted ${jsonColumnCount} protobuf columns to JSON`,
          );
        }
        ...
        if (expectedJsonColumns > 0 && i <= 10 && jsonColumnCount > 0) {
          expect(jsonColumnCount).toBeGreaterThanOrEqual(
            expectedJsonColumns,
...
describe("CSV Preprocessor - system.rangelog key analysis", () => {
  it("should process system.rangelog.txt and analyze start_key transformation", () => {
    ...
    console.log(
      "Available files in debug.zip:",
      Object.keys(files).filter((name) => name.includes("rangelog")),
    );

    // Try different possible names
    const rangelogFile =
      files["debug/system.rangelog.txt"] ||
      files["system.rangelog.txt"] ||
      files["system_rangelog.txt"] ||
      Object.keys(files).find(
        (name) => name.includes("rangelog") && name.endsWith(".txt"),
      );

    expect(rangelogFile).toBeDefined();
    const content = new TextDecoder("utf-8").decode(
      new Uint8Array(rangelogFile!),
    );
    ...
    for (let i = 0; i < Math.min(3, lines.length); i++) {
      ...
      while ((match = startKeyPattern.exec(line)) !== null) {
        ...
        if (
          /^[A-Za-z0-9+/]*(=|==)?$/.test(startKeyValue) &&
          startKeyValue.length % 4 === 0
        ) {
          console.log(
            `❌ Found untransformed base64 start_key: "${startKeyValue}"`,
          );
          foundBase64InStartKey = true;
        } else if (
          startKeyValue.startsWith("/Table/") ||
          startKeyValue.startsWith("/")
        ) {
