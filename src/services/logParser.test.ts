...
describe('LogParser', () => {
  ...
  it('should parse log entry with counter correctly', () => {
    const singleLine = 'I250917 21:50:59.410399 7012 4@util/log/event_log.go:90 ⋮ [T1,Vsystem,n1] 1 {"test": "message"}';
  ...
  it('should parse log entry without counter correctly', () => {
    const singleLine = 'I250917 21:50:59.410558 7012 util/log/file_sync_buffer.go:237 ⋮ [T1,config] file created at: 2025/09/17 21:50:59';
    const { entries } = parser.parseLogFile(singleLine, 'test.log');
  ...
  it('should parse tags with key-value pairs correctly', () => {
    const singleLine = 'I250917 21:50:59.410399 7012 4@util/log/event_log.go:90 ⋮ [T1,Vsystem,n1,client=127.0.0.1:39464,hostnossl,user=root] 1 test message';
    const { entries } = parser.parseLogFile(singleLine, 'test.log');

    expect(entries).toHaveLength(1);
    const entry = entries[0];

    expect(entry.tags).toEqual(['T1', 'Vsystem', 'n1', 'client', 'hostnossl', 'user']);
  ...
  it('should handle multi-line log entries', () => {
    ...
    expect(firstEntry.message).toContain('at async UserService.getUser(user.js:12)');
  ...
  it('should extract different log levels correctly', () => {
    const testCases = [
      { line: 'I250917 21:50:59.410558 7012 test.go:1 ⋮ [T1] info message', level: 'I' },
      { line: 'W250917 21:50:59.410558 7012 test.go:1 ⋮ [T1] warning message', level: 'W' },
      { line: 'E250917 21:50:59.410558 7012 test.go:1 ⋮ [T1] error message', level: 'E' },
      { line: 'F250917 21:50:59.410558 7012 test.go:1 ⋮ [T1] fatal message', level: 'F' }
    ];
