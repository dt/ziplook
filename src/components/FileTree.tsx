...
function FileTree({ entries, filter }: FileTreeProps) {
  ...
  const tree = useMemo(() => {
    ...
    if (filteredEntries.length > 0) {
      ...
      if (
        firstFolder &&
        filteredEntries.every((e) => e.path.startsWith(firstFolder + "/"))
      ) {
    ...
    filteredEntries.forEach((entry) => {
      ...
      const adjustedPath = commonPrefix
  ...
  const collectVisibleFiles = useCallback(
    (node: TreeNode, items: NavigationItem[] = []): NavigationItem[] => {
      if (node.name === "") {
  ...
  const renderNode = (node: TreeNode, level: number = 0) => {
    ...
    if (node.name === "") {
      ...
      return Array.from(node.children.values())
        ...
        .map((child) => <div key={child.path}>{renderNode(child, level)}</div>);
    }
    ...
    const isHighlighted =
      ...
      navigation.state.items[navigation.state.highlightedIndex]?.id ===
        `file-${node.entry.id}`;
    ...
    return (
      <div className="tree-node" key={node.path}>
        <div
          ref={
            isFile && node.entry
              ? (el) => registerElement(`file-${node.entry!.id}`, el)
              : undefined
          }
        ...
        >
          ...
          <span className="tree-label" title={node.path || node.name}>
            {node.name}
          </span>
          {isFile && node.entry && (
            <span className="tree-size">{formatFileSize(node.entry.size)}</span>
          )}
        </div>
