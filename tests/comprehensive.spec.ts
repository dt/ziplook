...
test.describe("Zip File Loading and Browsing", () => {
  test("should load zip file and display file tree structure", async ({
    page,
  }) => {
    await page.goto("/");
...
test.describe("File Viewing and Content Display", () => {
  test("should open and display text file content", async ({ page }) => {
    ...
    const textFile = page
      .locator(".tree-item.file")
      .filter({ hasText: ".txt" })
      .first();
    await textFile.click();

    // Verify file viewer opens
    await expect(page.locator(".enhanced-file-viewer")).toBeVisible({
      timeout: SHORT_TIMEOUT,
    });

    // Verify content area exists
    const contentArea = page
      .locator(".file-content-scroll, .file-lines")
      .first();
  ...
  test("should display line numbers for text files", async ({ page }) => {
    ...
    await page.waitForSelector(".enhanced-file-viewer", {
      timeout: SHORT_TIMEOUT,
    });
...
test.describe("Tab Management", () => {
  ...
  test("should close tabs", async ({ page }) => {
    ...
    const closeButton = tabs
      .first()
      .locator('.tab-close, .close-icon, button[aria-label*="Close"]')
      .first();
...
test.describe("SQL Tables and Queries", () => {
  test("should display tables in sidebar", async ({ page }) => {
    ...
    const tables = page
      .locator('.table-item, .table-entry, [role="treeitem"]')
      .filter({ hasText: /\w+/ });
    ...
    if (await firstTable.isVisible({ timeout: SHORT_TIMEOUT })) {
      ...
      if (await editor.isVisible({ timeout: SHORT_TIMEOUT })) {
        ...
        const suggestions = page
          .locator(".suggest-widget, .monaco-list")
          .first();
...
test.describe("CRDB-Specific Features", () => {
  ...
  test("should display spanconfigs if present", async ({ page }) => {
    ...
    const spanconfigFile = page
      .locator(".tree-item.file")
      .filter({ hasText: /span/i })
      .first();
...
test.describe("Error Handling", () => {
  test("should handle missing or invalid files gracefully", async ({
    page,
  }) => {
    await loadDebugZip(page);

    // Try to open a binary file if present
    const binaryFile = page
      .locator(".tree-item.file")
      .filter({ hasText: /\.(bin|exe|dll|so|dylib)/ })
      .first();
    if (await binaryFile.isVisible({ timeout: 1000 })) {
      ...
      const viewer = page
        .locator(".enhanced-file-viewer, .error-viewer")
        .first();
      await expect(viewer).toBeVisible({ timeout: SHORT_TIMEOUT });
    }
  });
